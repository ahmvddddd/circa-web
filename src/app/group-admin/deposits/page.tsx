...
export default function GroupAdminDepositsPage() {
  ...
  useEffect(() => {
    ...
    async function loadDeposits() {
      ...
      try {
        ...
        const apiDeposits: AdminDeposit[] = Array.isArray(json?.data)
          ? json.data
          : [];
  ...
  useEffect(() => {
    if (error === "UNAUTHENTICATED") {
      router.push("/login?next=/group-admin/deposits");
    }
  }, [error, router]);
  ...
  return (
    ...
    >
      <section className="rounded-xl border border-border bg-muted p-4 sm:p-6">
        ...
        {loading && (
          <p className="text-xs text-gray-500">Loading deposits...</p>
        )}
